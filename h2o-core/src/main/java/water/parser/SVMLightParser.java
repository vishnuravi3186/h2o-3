                ...
                if(c == ':'){
                  if(exp == 0 && number >= colIdx && (int)number == number){
                    ...
                    if(number <= colIdx)
                      ...
                      err = "Got non-integer as column id: " + PrettyPrint.pow(number,exp) + ". Rest of the line is skipped.";
                    else
  ...
  private static class SVMLightInspectParseWriter extends PreviewParseWriter {
    ...
    @Override public void addNumCol(int colIdx, long number, int exp) {
      ...
      if(colIdx < MAX_PREVIEW_COLS && _nlines < MAX_PREVIEW_LINES)
        _data[_nlines][colIdx] = Double.toString(PrettyPrint.pow(number,exp));
    }
